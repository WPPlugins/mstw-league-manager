	...
	class MSTW_LM_CSV_IMPORTER {
		...
		function form( ) {
			...
			<div class="wrap">
				...
				<!-- Adjust Game Date-Time timestamps -->
				<form class="add:the-list: validate" method="post" enctype="multipart/form-data" action="">
					<!-- Enter the time adjustment in hours -->
					<table class='form-table'>
						<thead><tr><th><?php echo __( 'Adjust ALL Game Times', 'mstw-league-manager' ) ?></th></tr></thead>
						
						<tr>  <!-- hour adjustment field -->
							<td><label for="hour_adjustment"><?php _e( 'Hour Adjustment:', 'mstw-league-manager' ); ?></label></td>
							<td><input name="hour_adjustment" id="hour_adjustment" type="text" value="0" aria-required="true" /><br/>
							<span class="description">Enter positive hours to move timezone east, negative hours to move west.</span>
							</td>
						</tr>

						<tr>
							<td><label for="league_to_adjust"><?php _e( 'League:', 'mstw-league-manager' ); ?></label></td>
							<td>
							  <?php 
							  $id = 'league_to_adjust'; 
							  $ret_value = mstw_lm_build_league_select( '', $id, true );
							  if ( -1 == $ret_value ) {
						...
						<tr> <!-- Submit button -->
						<td colspan="2" class="submit"><input type="submit" class="button" name="submit" value="<?php _e( 'Adjust Times', 'mstw-league-manager' ); ?>"/></td>
						</tr>
					</table>
				</form>
		...
		function post( $options ) {
			...
			switch( $options['submit_value'] ) {
				...
				*/
				...
				case __( 'Adjust Times', 'mstw-league-manager' ):
					mstw_log_msg( 'In post() method: Adjusting times ...' );
					
					if ( array_key_exists( 'hour_adjustment', $_POST ) && array_key_exists( 'league_to_adjust', $_POST ) ) {
						mstw_log_msg( "Time adjustment (hours): " . $_POST['hour_adjustment'] );
						mstw_log_msg( "League to adjust: " . $_POST['league_to_adjust'] );
						$hours = $_POST['hour_adjustment'];
						mstw_log_msg( "Time adjustment (hours): " . $hours );
						$league = $_POST['league_to_adjust'];
						
						if ( 0 == $hours ) {
							//mstw_log_msg( "Time adjustment = 0. Done." );
							$this->log['notice'][] = __( 'Time adjustment = 0. Nothing to do.','mstw-league-manager' );
						
						} else {
							$seconds = $hours * HOUR_IN_SECONDS;
							mstw_log_msg( "Time adjustment (seconds): " . $seconds );
							
							//Grab the games in the specified league
							...
							//mstw_log_msg( "Games found: " . $games_found );
							
							foreach ( $games as $game ) {
								$dtg = get_post_meta( $game -> ID, 'game_unix_dtg', true );
								//mstw_log_msg( 'Original DTG: ' . date( 'Y-m-d H:i', $dtg ) );
								//mstw_log_msg( 'Original DTG: ' . $dtg );
								$dtg += $seconds;
								//mstw_log_msg( 'New DTG: ' . date( 'Y-m-d H:i', $dtg ) );
								//mstw_log_msg( 'New DTG: ' . $dtg );
								update_post_meta( $game->ID, 'game_unix_dtg', $dtg );
							}
							
							$this->log['notice'][] = sprintf( __( 'Adjusted %s games by %s hours.','mstw-league-manager' ), $games_found, $hours );
						...
						mstw_log_msg( "No time adjustment specified." );
						$this->log['error'][] = __( 'No time adjustment specified.','mstw-league-manager' );
					}
