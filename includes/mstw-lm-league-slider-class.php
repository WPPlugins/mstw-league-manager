<?php
 /*---------------------------------------------------------------------------
 *	mstw-lm-league-schedule-slider-class.php
 *	Contains the class for the MSTW League Manager League Schedule Slider
 *  shortcode [mstw_league_schedule_slider]
 *
...
//---------------------------------------------------------------------------
// Add the shortcode handler, which will create the League Schedule Slider
// on the user side. Handles the shortcode parameters, if there were 
// any, then calls mstw_lm_build_tourney_bracket( ) to create the output
// 

class MSTW_LEAGUE_SCHEDULE_SLIDER {
	
	public function __construct( ) {
		add_filter( 'get_league_schedule_slider_instance', [$this, 'get_instance'] );
	}
	...
	//
	public function league_schedule_slider_handler( $atts, $content = null, $shortcode ) {
		//mstw_log_msg( "MSTW_LEAGUE_SCHEDULE_SLIDER:league_schedule_slider_handler: shortcode= $shortcode " );
		...
		//mstw_log_msg( 'last slash position: ' . $last_slash );
		
		//$tourney_slug = $atts['tourney'];
		//mstw_log_msg( "Tournament (slug): $tourney_slug" );
		
		// get the options set in the admin settings screen
		if ( 'mstw_league_schedule_slider' == $shortcode ) {
			$args = mstw_lm_get_slider_defaults( );
			
		} else if ( 'mstw_league_schedule_ticker' == $shortcode ) {
			$args = mstw_lm_get_ticker_defaults( );
	...
	} //End: league_schedule_slider_handler( )
	
	//-----------------------------------------------------------------------------
	// build_games_list - Builds the list of games based on the slider atts 
	// 	Called from league_schedule_slider_handler( ) after it has processed 
	...
	function build_games_list( $atts, $type = 'all' ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_games_list:' );
		//mstw_log_msg( '$atts:' );
		//mstw_log_msg( $atts );
		...
		if ( $interval_days = $atts['interval_days'] ) {
			if ( is_numeric( $interval_days ) ) {
				...
				mstw_log_msg( "MSTW_LEAGUE_SCHEDULE_SLIDER.build_games_list: interval_days is not numeric." );
	...
	function build_shortcode( $shortcode, $atts, $games ) { 
		//mstw_log_msg( "MSTW_LEAGUE_SCHEDULE_SLIDER.build_shortcode: $shortcode" );
		...
		if ( 'mstw_league_schedule_slider' == $shortcode ) {
			...
			$output = $this -> build_ticker_html( $games, $games_to_show, $game_number, $atts );
	...
	//-----------------------------------------------------------------------------
	// build_slider_html - returns the league schedule slider HTML
	// 	
	// ARGUMENTS
	...
	function build_slider_html( $games, $games_to_show, $game_number, $atts ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_slider_html:' );
		//mstw_log_msg( $games );
	...
	//-----------------------------------------------------------------------------
	// build_ticker_html - returns the league schedule ticker HTML
	// 	
	// ARGUMENTS
	// 	$games  - list of games (objects/CPTs)
	//	$games_to_show - number of games shown in one "ticker width"
	//  $game_number - number of game in $games that goes furthest right (visible)
	//  $atts - shortcode arguments (combined with settings)
	//
	// RETURNS
	//	$output - html for the ticker
	//
	function build_ticker_html( $games, $games_to_show, $game_number, $atts ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_ticker_html:' );
		//mstw_log_msg( $games );
		//mstw_log_msg( 'atts:' );
		//mstw_log_msg( $atts );
		...
		$dummy = $nbr_of_games - $games_to_show + 1 ;
		
		//mstw_log_msg( '$nbr_of_games - $games_to_show + 1: ' . $dummy );
		...
		//mstw_log_msg( "view_width= $container_width " );
		...
		$output = "\n<div class='lmt-ticker-container' id='lm-ticker-container$css_tag' style='width: {$container_width}px;' >\n";
		
			//ticker header
			if( $atts['show_header'] ) {
				$output .= "<div class='lmt-header' id='lmt-header$css_tag'>\n";
				$output .= $this -> build_ticker_header( $atts );
				$output .= "</div> <!-- .lmt-header --> \n";
			}
			// holds next and prev buttons and ticker content (game blocks)
			$output .= "<div class='lmt-ticker-holder'>\n";
			
				$output .= "<div class='lmt-prev' id='lmt-prev$css_tag'>\n";
				$output .= "</div> <!-- .lmt-prev --> \n";

				$output .= "<div class='lmt-viewport lmt-viewport$css_tag' style='width: {$view_width}px;'>\n";
				// contains the ticker content (game blocks)
				  $output .= "<div class='lmt-ticker-content' id='lmt-ticker-content$css_tag'>\n";
				    ...
				    $output  .= "</ul>\n";
				  $output .= "</div> <!-- .lmt-ticker-content --> \n";
				
				$output .= "</div> <!-- .lmt-ticker-viewport --> \n";
				
				$output .= "<div class='lmt-next' id='lmt-next$css_tag'>\n";
				$output .= "</div> <!-- .lmt-next --> \n";
				
			
			$output .= "</div> <!-- .lmt-holder --> \n";
		
		$output .= "</div> <!-- .lm-schedule-ticker --> \n";
		
		return $output;
		
	} //End: build_ticker_html
	
	//---------------------------------------------------------------------------
	// build_ticker_header
	// 	Builds the league schedule ticker header 
	//
	// ARGUMENTS:
	...
	function build_ticker_header( $atts ) {
		...
		if( $atts['show_header'] ) {
			$ret .= "<div class=lmt-title>\n";
				$ret .= $atts['title'];	
			$ret .= "</div> <!-- .lmt-title -->\n";
			
			$ret .= "<div class=lmt-link>\n";
				if( '' != $atts['link_label'] ) {
					if( '' != $atts['link'] ) {
			...
			if( $atts['show_message'] ) {
				...
				$ret .= "<div class=lmt-message>$msg</div>";
			}
	...
	// 	Builds the header for the league schedule ticker game block 
	//
	// ARGUMENTS:
	//	$atts - the combined defaults, settings, and shortcode atts
	...
	function build_ticker_game_header( $atts, $game, $game_status ) {
		...
		$ret = "<div class=lmt-game-header> \n";
		$ret .= "<p class='lmt-status'>\n";
		...
		if( 'is_final' == $game_status['status'] ) {
			$ret .= __( 'Final', 'mstw-league-manager' ) . "&nbsp;&nbsp;" . date( 'd M', $game_status['start_dtg'] );
	...
	function build_ticker_sport_line( $atts, $game ) {
		...
		if ( $atts['show_sport'] ) {
			$ret .= "<div class='lmt-sport'>\n";
			//$ret .= "<p class='lmt-sport'>\n";
	...
	//-----------------------------------------------------------------------------
	// build_ticker_team_line - returns the HTML for a ticker team line
	// 	
	// ARGUMENTS
	...
	function build_ticker_team_line( $team = 'home', $game, $status, $atts ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_ticker_team_line:' );
	...
	function build_game_block( $game, $atts, $css_tag ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_game_block:' );
	...
	function build_date_line( $game, $atts, $css_tag ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_date_line:' );
	...
	function build_sport_line( $game, $atts, $css_tag ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_sport_line:' );
	...
	function build_matchup_block( $game, $atts, $css_tag ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_matchup_block:' );
	...
	function build_team_line( $team = 'home', $game, $status, $atts, $css_tag ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_team_line:' );
	...
	function get_game_status( $game ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.get_game_status:' );
	...
	function build_venue_line( $game, $atts, $css_tag ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_venue_line:' );
	...
	function build_time_status_line( $game_status, $atts, $css_tag ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.build_time_status_line:' );
	...
	function build_scroll_controls( $css_tag, $slider_view_height ) {
		//mstw_log_msg( "MSTW_LEAGUE_SCHEDULE_SLIDER.build_scroll_controls:")
	...
	function get_next_game( $games, $dtg = null ) {
		//mstw_log_msg( 'MSTW_LEAGUE_SCHEDULE_SLIDER.get_next_game:' );
		//mstw_log_msg( $games );
...
} //End: class MSTW_LEAGUE_SCHEDULE_SLIDER
...
//
$league_slider = new MSTW_LEAGUE_SCHEDULE_SLIDER;

add_shortcode( 'mstw_league_schedule_slider', array( $league_slider, 'league_schedule_slider_handler' ) );

add_shortcode( 'mstw_league_schedule_ticker', array( $league_slider, 'league_schedule_slider_handler' ) );

add_shortcode( 'mstw_league_schedule_scoreboard', array( $league_slider, 'league_schedule_slider_handler' ) );